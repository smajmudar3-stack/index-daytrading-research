...
MAX_TRADE_COST_PCT = 12.0     # total slippage as a share of max risk
...
def _vertical(calls, puts, spot, dte, ladder, inc, bullish, debit):
    ...
    # most to learn.
    cost = lq["half_spread"] + sq["half_spread"]
...
def _exits(struct, trend, direction, spot, implied_move):
    ...
    if struct.get("calendar"):
        target = "close at +30% on the debit, or the morning after the catalyst"
        stop = "close at -50% of the debit"
    elif struct["debit"]:
        target = (f"close at {struct['width'] * 0.6:.2f} "
                  f"(60% of the ${struct['width']:g} width)")
        stop = f"close at -50% of the {struct['net']:.2f} debit"
    else:
        target = f"buy it back at {struct['net'] * 0.45:.2f} (keep ~55% of the credit)"
        stop = f"close at 2x the credit, or if {struct['short_strike']:g} trades through"
