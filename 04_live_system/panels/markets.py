...
def swing():
    ...
    for s in sigs[:12]:
        rows.append({"k": s.get("ticker", "?"),
                     "v": f"{s.get('direction', '')} · conviction {s.get('conviction', '—')}"})
...
def blackswan():
    ...
    rows = [
        {"k": "unfiltered far-OTM", "v": "−45.6%", "tier": "measured", "severity": "stop"},
        {"k": "spread ≤ 20% only", "v": "+5.6%", "tier": "measured"},
        {"k": "16–30 delta band", "v": "+26.1%", "tier": "measured"},
        {"k": "far tail", "v": "−90%", "tier": "measured", "severity": "stop"},
    ]
    ...
    return panel("blackswan", "Far-OTM convexity", state=OK, body={"rows": rows}, note=note,
