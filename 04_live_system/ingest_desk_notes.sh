...
LAST="$("$PY" desk_notes.py --last-ingested 2>/dev/null | tail -1)"
echo "last ingested: ${LAST:-<none>}" >>"$LOG"
...
2. The overlay's newest ingested note is dated: ${LAST:-none yet}
   Skip any note at or before that timestamp. If there is nothing newer, print
   "no new notes" and stop without writing anything.

3. For each new note, OLDEST FIRST, read the full plain-text body and structure it into
