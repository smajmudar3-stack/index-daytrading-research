...
FULL = f"opt_eod/{SYM}_monthly_full.parquet"  # path under DATA_ROOT, resolved at the read site
...
def main():
    df = pd.read_parquet(paths.require_data(FULL))
    ...
    t.to_parquet(paths.data("opt_eod", f"{SYM}_hedged_{TARGET_DTE}.parquet"))
    print("\nsaved trade table")
