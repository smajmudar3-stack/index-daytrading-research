...
# ---------------------------------------------------------------- 0DTE SPX
SPXW = "spxw/data_opt.parquet"  # path under DATA_ROOT, resolved at the read site
...
# ---------------------------------------------------------------- SPY weekly / monthly
OPT = "opt_eod/SPY_options.parquet"
...
def main():
    ...
    df = pd.read_parquet(paths.require_data(SPXW), columns=["quote_date", "quote_time", "option_type", "mnes_rel",
                                                            "mid", "bas", "delta"])
