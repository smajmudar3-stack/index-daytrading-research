...
B = "opt_eod"  # path under DATA_ROOT, resolved at the read site
...
def main():
    opt = pd.read_parquet(
        paths.require_data(B, "SPY_options.parquet"),
        columns=["date", "expiration", "strike", "type", "bid", "ask",
                 "implied_volatility", "delta", "volume", "open_interest"])
    u = pd.read_parquet(paths.require_data(B, "SPY_underlying.parquet"), columns=["date", "close"])
    ...
    out.to_parquet(paths.data(B, "SPY_ivfeatures.parquet"))
