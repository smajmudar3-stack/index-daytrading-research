...
For every (week, name) in the cross-sectional panel with a direction from a chosen signal:
  ...
  exits       expiry   : payoff off the split-adjusted close on the expiry date. Exact.
              ...
              CAVEAT: the thinned chain keeps ~11 strikes around spot, so a leg is still
              quoted at t+H mostly when spot has moved TOWARD it. For a credit spread that
              selects the losing trades; for a debit spread the winning ones. The time-exit
              rows are therefore biased and are reported as bounds, not as results. The
              expiry row is exact.
...
def pick_legs(ch, entries):
    ...
    for struct, right_of_side, d_long, d_short in (
        ...
        # The long leg must be the NEARER strike on a debit and the FARTHER on a credit, or
        # the "spread" is inverted and its payoff can exceed its width. Delta is not
        # perfectly monotone in strike on a thinned chain with stale far-wing IVs, so this
        # is enforced on strikes, not assumed from delta.
        is_call = s.call_put == "Call"
        if struct == "debit":
            ok = np.where(is_call, s.strike_l < s.strike_s, s.strike_l > s.strike_s)
        else:
            ok = np.where(is_call, s.strike_l > s.strike_s, s.strike_l < s.strike_s)
        s = s[ok]
        s["struct"] = struct
...
def main():
    ...
    # ONE YEAR AT A TIME. Loading all 90 months of chain at once is ~6 GB and the machine
    # swapped; each year plus two months of spillover for settlement is a tenth of that.
    parts = []
    for yr in sorted({d.year for d in ent.date.unique()}):
        e = ent[ent.date.dt.year == yr]
        months = [f"{yr}-{m:02d}" for m in range(1, 13)] + [f"{yr+1}-01", f"{yr+1}-02"]
        ch = load_chain(months)
        if ch.empty:
            continue
        t = pick_legs(ch, e)
        parts.append(settle(t, ch, px))
        print(f"{yr}: {len(parts[-1]):,} trades", flush=True)
        del ch
